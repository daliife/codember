const encryption_policies = `8-10 r: ozrcdfnug
9-10 q: hvsazxrigf
1-9 j: bbqonxjt
...
const inputArray = encryption_policies.split("\n");
let validPasswords = 0;
let invalidPasswordsList = [];
for (let i = 0; i < inputArray.length; i++) {
  const password = String(inputArray[i].split(":")[1]);
  const policy = inputArray[i].split(":")[0];
  const min = policy.split(" ")[0].split("-")[0];
  const max = policy.split(" ")[0].split("-")[1];
  ...
  if (count >= min && count <= max) {
    ...
    invalidPasswordsList.push(password);
  }
}
console.log(validPasswords); //21
console.log(invalidPasswordsList[41]); //bqamidgewtbuz
