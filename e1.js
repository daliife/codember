...
const animalsToLower = animals.toLowerCase();

// Create an array with all the animals
const animalsArray = animalsToLower.split(" ");

// Create an object with the animals and the number of times they appear in the string
const animalCounter = animalsArray.reduce((acc, animal) => {
  if (!acc[animal]) {
    acc[animal] = 1;
  } else {
    acc[animal] += 1;
...
const result = Object.entries(animalCounter).reduce((acc, [key, value]) => {
  acc += `${key}${value}`;
  return acc;
}, "");
